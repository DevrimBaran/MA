...
const PERFORMANCE_TEST: bool = true; // Set to false for testing
...
trait BenchMpscQueue<T: Send>: Send + Sync + 'static {
   fn bench_push(&self, item: T) -> Result<(), ()>; // Standardized error for benchmark
   fn bench_pop(&self) -> Result<T, ()>;      // Standardized error for benchmark
...
// Implement for DrescherQueue (or any MpscQueue impl)
...
where
   Q: BenchMpscQueue<usize> + 'static, // Use BenchMpscQueue here
   F: FnOnce() -> (&'static Q, *mut u8, usize),
{
   ...
   let (q, shm_ptr, shm_size) = queue_init_fn(); // q is &'static Q
      ...
      match unsafe { fork() } {
         Ok(ForkResult::Child) => {
               for i in 0..items_per_producer {
                  ...
                  while q.bench_push(item_value).is_err() { // Use bench_push
                     std::hint::spin_loop();
                  }
               }
   ...
   while consumed_count < total_items_to_produce {
      if let Ok(_item) = q.bench_pop() { // Use bench_pop
         consumed_count += 1;
      } else {
         if producer_sync.producers_done_count.load(Ordering::Acquire) == num_producers as u32 {
               ...
               if q.bench_is_empty() { // Use bench_is_empty
                  break;
               }
         }
         std::hint::spin_loop();
   ...
   if (consumed_count < total_items_to_produce) && (PERFORMANCE_TEST == false) {
      eprintln!(
         "Warning: MPSC Consumer consumed {}/{} items. Queue type: DrescherQueue",
