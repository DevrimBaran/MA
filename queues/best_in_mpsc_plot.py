...
MPSC_QUEUE_ALGORITHMS = ["DQueue (Native MPSC)", "YMC (MPMC as MPSC)"]
...
def load_benchmark_data(base_path, queue_algo_name, num_producers):
    ...
    try:
        ...
        print(
            f"Warning: Could not decode JSON for '{benchmark_id}' at '{path_segment}'"
        )
        return None
    except Exception as e:
        print(
            f"Warning: Error loading data for '{benchmark_id}' from '{path_segment}': {e}"
        )
        return None
...
def main():
    ...
    for num_producers in PRODUCER_COUNTS:
        ...
        for queue_algo in MPSC_QUEUE_ALGORITHMS:
            samples_ns = load_benchmark_data(
                CRITERION_BASE_PATH, queue_algo, num_producers
            )

            if samples_ns is not None and len(samples_ns) > 0:
                print(
                    f"  Successfully loaded {len(samples_ns)} samples for {queue_algo} with {num_producers}P1C."
                )
                ...
                print(
                    f"  No data found or loaded for {queue_algo} with {num_producers}P1C."
                )

        if not current_producer_count_data:
            print(
                f"Error: No benchmark data found for {num_producers}P1C. Skipping violin plot."
            )
            continue
        ...
        plot_title_violin = f'MPSC Performance Comparison ({num_producers} Producer{"s" if num_producers > 1 else ""}, 1 Consumer, {num_producers * ITEMS_PER_PRODUCER:,} Total Items)'
    ...
    for algo in MPSC_QUEUE_ALGORITHMS:
        ...
        ax.plot(
            algo_data["Producer Count"],
            algo_data["Mean Time (µs)"],
            marker="o",
            markersize=8,
            linewidth=2,
            label=algo,
    ...
    plt.title(
        "MPSC Performance: Mean Execution Time vs Number of Producers", fontsize=14
    )
