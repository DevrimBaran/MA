...
def main():
    ...
    for num_consumers in CONSUMER_COUNTS:
        ...
        for queue_algo in SPMC_QUEUE_ALGORITHMS:
            samples_ns = load_benchmark_data(
                CRITERION_BASE_PATH, queue_algo, num_consumers
            )
            ...
            if samples_ns is not None and len(samples_ns) > 0:
                ...
                print(
                    f"  No data found or loaded for {queue_algo} with 1P{num_consumers}C."
                )

        if not current_consumer_count_data:
            print(
                f"Error: No benchmark data found for 1P{num_consumers}C. Skipping violin plot."
            )
            continue
    ...
    plt.title(
        "SPMC Performance: Mean Execution Time vs Number of Consumers", fontsize=14
    )
