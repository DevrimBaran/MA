...
def main():
    ...
    ax = sns.violinplot(x='Queue Type', y=Y_AXIS_LABEL, data=df, 
                        palette='Set3', cut=0, inner='quartile', scale='width')
    ...
    plt.text(0.5, 0.01, "Test: 1 Producer, 1 Consumer, 10K iterations", 
             ha='center', transform=plt.gcf().transFigure, fontsize=10, style='italic')
    ...
    mean_df = pd.DataFrame({
        'Queue Type': list(mean_values_dict.keys()),
        'Mean Time (µs)': list(mean_values_dict.values())
    })
    mean_df = mean_df.sort_values('Mean Time (µs)')
    
    bars = plt.bar(mean_df['Queue Type'], mean_df['Mean Time (µs)'], color=['#8dd3c7', '#ffffb3', '#bebada', '#fb8072'])
    ...
    plt.title('Mean Performance Comparison - Best Algorithms in SPSC', fontsize=14)
    plt.xlabel('Queue Algorithm', fontsize=12)
    ...
    bar_output_file = OUTPUT_PLOT_FILE.replace('.png', '_bar.png')
