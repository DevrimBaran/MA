...
# as 'Execution Time per Iteration' from Criterion refers to one run of fork_and_run_mpsc.
...
def load_benchmark_data(base_path, queue_group_name, num_producers, items_per_producer):
   ...
   path_segment = os.path.join(base_path, queue_group_name, benchmark_id_suffix, 'new', 'sample.json')
   ...
   try:
      ...
      print(f"Warning: Could not decode JSON for '{queue_group_name}/{benchmark_id_suffix}' at '{path_segment}'")
      return None
   except Exception as e:
      print(f"Warning: Error loading data for '{queue_group_name}/{benchmark_id_suffix}' from '{path_segment}': {e}")
      return None
...
def main():
   ...
   for num_prods in PRODUCER_COUNTS:
      ...
      for queue_group in MPSC_QUEUE_GROUPS:
         samples_ns = load_benchmark_data(CRITERION_BASE_PATH, queue_group, num_prods, items_this_run_per_producer)
         
         if samples_ns is not None and len(samples_ns) > 0:
               print(f"  Successfully loaded {len(samples_ns)} samples for {queue_group} with {num_prods} producer(s).")
               ...
               all_benchmark_data.append({
                  ...
                  'Mean Time (ns)': np.mean(samples_ns), # Mean of total durations
                  'Times (ns)': samples_ns # Keep all samples for potential further analysis
               })
         else:
               print(f"  No data found or loaded for {queue_group} with {num_prods} producer(s).")

      if not current_producer_count_data:
         print(f"Error: No benchmark data found for {num_prods} producer(s). Skipping violin plot.")
         continue
   ...
   for record in all_benchmark_data:
      # Calculate mean time per operation (total items = Producer Count * ITEMS_PER_PRODUCER)
      # This gives a normalized view if desired, or we can plot total time.
      # Let's plot mean total execution time first, as it's directly from Criterion's iteration.
   ...
   plt.ylabel('Mean Total Execution Time (µs)', fontsize=12) # Total time for all items
