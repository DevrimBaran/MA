...
impl<T: Send + Clone + 'static> SDPWFQueue<T> {
    ...
    pub fn enqueue(&self, thread_id: usize, item: T) -> Result<(), ()> {
        ...
        unsafe {
            ...
            if self.use_improved_traversal() {
                if let Ok(()) =
                    self.enqueue_with_improved_traversal(thread_id, &item, start_idx, end_idx)
                {
                    return Ok(());
                }
            } else {
            ...
            if self.helping_queue_offset > 0 {
                if let Ok(()) = self.enqueue_with_helping_queue(thread_id, &item) {
                    return Ok(());
                }
            }

            // Full traversal of entire queue
            let mut attempts = 0;
            const MAX_ATTEMPTS: usize = 3; // Multiple rounds to ensure we don't miss freed elements

            while attempts < MAX_ATTEMPTS {
                let mut current = 0; // Start from beginning for full traversal
                for _ in 0..self.total_elements {
                    if self.try_enqueue_at(current, &item)? {
                        return Ok(());
                    }
            ...
            Err(()) // Queue is truly full
        }
    }

    // Try to enqueue at a specific element (following Figure 3 flowchart)
    unsafe fn try_enqueue_at(&self, idx: usize, item: &T) -> Result<bool, ()> {
        ...
        match (*elem).status.compare_exchange(
            ...
            Err(_) => Ok(false), // Failed to set init bit
        }
    }

    // Enqueue with improved traversal algorithm (Section V)
    ...
    // Enqueue using helping queue (Section IV)
    unsafe fn enqueue_with_helping_queue(&self, thread_id: usize, item: &T) -> Result<(), ()> {
        // Multiple passes through helping queue
        for _ in 0..2 {
    ...
    // Algorithm from Figure 5: Dequeue operation
    pub fn dequeue(&self, thread_id: usize) -> Result<T, ()> {
        ...
        unsafe {
            ...
            for attempt in 0..3 {
                ...
                for _ in 0..self.total_elements {
                    if let Some(item) = self.try_dequeue_at(current)? {
    ...
    // Try to dequeue from a specific element (following Figure 5 flowchart)
