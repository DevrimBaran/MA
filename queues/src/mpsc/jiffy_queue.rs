...
impl<T: Send + 'static> SharedPools<T> {
    ...
    unsafe fn alloc_bl_meta_with_node_array(
        ...
        loop {
            ...
            if self.bl_meta_free_list_head.compare_exchange(
                ...
                if node_array_ptr.is_null() {
                    let mut current_free_head = self.bl_meta_free_list_head.load(Ordering::Acquire);
                    loop { 
                        (*head).next_free_meta.store(current_free_head, Ordering::Release);
                        match self.bl_meta_free_list_head.compare_exchange(
                            current_free_head, head, Ordering::AcqRel, Ordering::Acquire ) {
                            Ok(_) => break,
                            Err(new_head_val) => current_free_head = new_head_val,
                        }
                    }
                    return ptr::null_mut();
                }
    ...
    unsafe fn dealloc_bl_meta_to_pool(&self, bl_meta_ptr: *mut BufferList<T>) {
        ...
        loop {
            ...
            match self.bl_meta_free_list_head.compare_exchange(
                current_head, bl_meta_ptr, Ordering::Release, Ordering::Relaxed,
            ) {
                Ok(_) => break,
...
impl<T: Send + 'static> JiffyQueue<T> {
    ...
    fn actual_enqueue(&self, data: T) -> Result<(), T> { 
        ...
        loop {
            ...
            if item_global_location >= tail_bl_end_loc {
                ...
                if tail_bl_ref.curr_buffer.is_null() { return Err(data); }
    ...
    unsafe fn attempt_fold_buffer(&self, bl_to_fold_ptr: *mut BufferList<T>) -> (*mut BufferList<T>, bool) {
        ...
        match prev_bl_ref.next.compare_exchange(
            ...
            Ok(_) => {
                ...
                // self.pools().dealloc_node_array_slice(bl_to_fold_mut.curr_buffer); // No-op

                let mut current_garbage_head = self.garbage_list_head.load(Ordering::Relaxed);
                loop {
                    (*bl_to_fold_ptr).next_in_garbage.store(current_garbage_head, Ordering::Relaxed);
    ...
    fn actual_process_garbage_list(&self, new_head_buffer_pos_threshold: u64) {
        ...
        if !still_deferred_list_head.is_null() {
            let mut current_global_garbage_head = self.garbage_list_head.load(Ordering::Relaxed);
            let mut temp_tail_of_still_deferred = still_deferred_list_head; 
            while !unsafe{(*temp_tail_of_still_deferred).next_in_garbage.load(Ordering::Relaxed)}.is_null() {
                temp_tail_of_still_deferred = unsafe{(*temp_tail_of_still_deferred).next_in_garbage.load(Ordering::Relaxed)};
            }

            loop {
                unsafe { (*temp_tail_of_still_deferred).next_in_garbage.store(current_global_garbage_head, Ordering::Relaxed); }
                match self.garbage_list_head.compare_exchange(
                    current_global_garbage_head, still_deferred_list_head, 
                    Ordering::Release, Ordering::Relaxed, 
                ) {
                    Ok(_) => break,
                ...
                'find_initial_temp_n: loop {
                    ...
                    while scan_idx < search_bl.capacity {
                        ...
                        if candidate_node_state == NodeState::Set as usize {
                            ...
                            if item_bl_ref.curr_buffer.is_null() { continue 'retry_dequeue; }
                            let item_node_ptr = unsafe { item_bl_ref.curr_buffer.add(final_temp_n_idx) };
                            let item_node_ref = unsafe { &*item_node_ptr }; 
                            
                            if item_node_ref.is_set.compare_exchange(
                                NodeState::Set as usize, NodeState::Handled as usize, Ordering::AcqRel, Ordering::Acquire
                            ).is_ok() { 
                                let data = unsafe { ptr::read(&(*item_node_ref).data).assume_init() };
                                return Some(data);
                            } else { continue 'retry_dequeue; } 
                        } 
                        scan_idx += 1;
                    } 
                    ...
                    if !found_set_in_search_bl && buffer_just_scanned_ptr != current_bl_ptr { 
                        ...
                        if is_fully_handled {
                            ...
                            if folded {
                                next_bl_for_scan = next_after_fold; 
                            }
                        }
                    }
...
impl<T: Send + 'static> MpscQueue<T> for JiffyQueue<T> { 
    ...
    fn is_empty(&self) -> bool {
        ...
        if head_bl.curr_buffer.is_null() { 
        ...
        // head_global_loc >= tail_loc
        // This means consumer is at or past where producers have announced items.
        // We must check the current node's state carefully.
        // Ensure curr_buffer is not null before adding, though earlier checks should cover it.
        if head_bl.curr_buffer.is_null() { return true; } // Should have been caught by the first check. Defensive.

        let node = unsafe { &*head_bl.curr_buffer.add(head_idx) };
        let state = node.is_set.load(Ordering::Acquire);
    
        if state == NodeState::Set as usize {
            return false; // Current node is set, so not empty.
        }
    
        if state == NodeState::Handled as usize {
            return head_bl.next.load(Ordering::Acquire).is_null() &&
                   head_bl_ptr == self.tail_of_queue.load(Ordering::Acquire);
        }
    
        // If state is Empty (and head_global_loc >= tail_loc), we conservatively say "not empty"
        // to give pop() a chance to retry and perform its scan.
        if state == NodeState::Empty as usize {
            return false;
        }
        
        return false; // Default to not empty in ambiguous situations (e.g. unknown state)
    }
